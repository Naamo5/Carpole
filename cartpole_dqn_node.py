...
class DQNAgent:
    ...
    def __init__(self, state_size, action_size,nbr_node):
...
###############################################################################
    ...
    def build_model(self):
        model = Sequential()
        model.add(Dense(self.nbr_node, input_dim=self.state_size, activation='relu',
...
if __name__ == "__main__":
    ...
    mean_score_last_100 = []
    for nbr_node in NODE:
        #Create agent, see the DQNAgent __init__ method for details
        agent = DQNAgent(state_size, action_size, nbr_node)
                ...
                if done:
                    ...
                    if agent.check_solve and len(scores)>100:
                        if np.mean(scores[-min(100, len(scores)):]) >= 195:
                            ...
                            sys.exit()
        mean_score_last_100.append(np.mean(scores[-min(100, len(scores)):]))
        #agent.plot_data(episodes,scores,max_q_mean)
    np.savetxt('mean_score.data', mean_score_last_100)
